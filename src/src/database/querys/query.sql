select distinct sub.id, sub.first_name, sub.last_name, sub.description, sub.progression 

FROM 

	( select users.id, users.first_name, users.last_name, evaluations.description, evaluations.progression, evaluations.score_needed, evaluations.grade_b,
  ...
  from evaluations_attempts join users on evaluations_attempts.user_id=users.id 
	...
	where evaluations_attempts.start_time > '2016-09-10 09:28:27.777635' AND evaluations_attempts.evaluations_id != 1  AND users.room_id = 24 AND evaluations.progression > 0.9 
	group by evaluations_attempts, evaluations.progression, evaluations.description, users.id, users.first_name, users.last_name, 	
	evaluations.score_needed, evaluations.grade_b) 

sub WHERE sub.total_answered >= sub.score_needed AND sub.inner_grade >= sub.grade_b order by sub.last_name, sub.progression;
