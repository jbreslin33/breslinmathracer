...
{
    dropDownName.value = dropDownValue;
}
...
function validateTextAreaContentLength(ta,e,maxLength, sequenceNo) {    
    if(ta.value.length >= maxLength) {
        ta.value = ta.value.substring(0, maxLength);
        e.preventDefault();
    }
    var isScoreOrCommentsChanged = document.getElementsByName('isScoreOrCommentsChanged' + sequenceNo);
    isScoreOrCommentsChanged[0].value = 'true';
}

// tkosirog - TTP 28416 - assign points to a specific text box.
function AssignRubricPointsFromDropDown(points, sequenceNo)
{
    var pointsTextBox = document.getElementById('StudentItemPointsText' + sequenceNo);
...
function submitAndRedirect(redirectUrl){
    ...
    var recalculate = document.getElementsByName('Recalculate');
    ...
    if (redirectUrl == '')
        recalculate[0].value = "true";

    document.getElementsByTagName('form')[0].submit();    
}

function clearInProcessERScoresComments(){
    var results = window.confirm('Are you sure you want to Cancel  your scoring?\nClick "OK" to leave the scoring workflow.  Or click "Cancel" to return to the scoring page, so you can save your work.')
    with (document.forms[0])
    {
        if(!results)
        {
