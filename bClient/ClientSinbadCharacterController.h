...
public:
	...
	void simulateKeyDown(int key)
	{
		...
		//LogString("simulateKeyDown");

		if (key == OIS::KC_Q && (mTopAnimID == ANIM_IDLE_TOP || mTopAnimID == ANIM_RUN_TOP))
		{
			// take swords out (or put them back, since it's the same animation but reversed)
			setTopAnimation(ANIM_DRAW_SWORDS, true);
			mTimer = 0;
		}
		else if (key == OIS::KC_E && !mSwordsDrawn)
		{
			if (mTopAnimID == ANIM_IDLE_TOP || mTopAnimID == ANIM_RUN_TOP)
			{
				// start dancing
		...
		// keep track of the player's intended direction
		else if (key == OIS::KC_W) mKeyDirection.z = -1;
		else if (key == OIS::KC_A) mKeyDirection.x = -1;
		else if (key == OIS::KC_S) mKeyDirection.z = 1;
		else if (key == OIS::KC_D) mKeyDirection.x = 1;
/*
		else if (evt.key == OIS::KC_SPACE && (mTopAnimID == ANIM_IDLE_TOP || mTopAnimID == ANIM_RUN_TOP))
		{
			// jump if on ground
...
*/
	...
	{
		// keep track of the player's intended direction
		if (key == OIS::KC_W && mKeyDirection.z == -1) mKeyDirection.z = 0;
		else if (key == OIS::KC_A && mKeyDirection.x == -1) mKeyDirection.x = 0;
		else if (key == OIS::KC_S && mKeyDirection.z == 1) mKeyDirection.z = 0;
		else if (key == OIS::KC_D && mKeyDirection.x == 1) mKeyDirection.x = 0;
